    ...
    :Controller()
{
    ...
    qDebug() << "old controller has "<< controller.m_admins.count()      << " admins in its list"     << endl;
...
void Controller::createTable()
{
    ...
    qry.clear();
    //==============================================================
    // About creating the Inventory table
    QString createInventoryTable =
    "create table IF NOT EXISTS inventory(                             "
    "item         varchar(50) primary key,                         "
    "price        float not null                                   "
    ");                                                            ";
    if(!qry.exec(createInventoryTable))
    {
        qDebug() << "error creating inventory table" << endl;
    }
    qry.clear();
...
void Controller :: createInventoryTable(QString item,float price)
{
    QSqlQuery qry;
    qry.prepare("insert into inventory   (    "
                "item,                        "
                "price)                       "
                "values(?,?);                 "
                );
    ...
    if(!qry.exec())
    {
         qDebug() << "Error adding inventory" << endl;
    }
...
void Controller ::updateInventory(QString item, float price)
{
    QSqlQuery qry;
    qry.prepare("update inventory set    "
                "item               = ?, "
                "price              = ? "
                "where item         = ?; "
                     );
...
QSqlQueryModel *Controller::getInventoryQueryModel()
{
    QSqlQueryModel* model = new QSqlQueryModel();
...
bool Controller::readRecordFile()
{
    ...
    while (!in.atEnd())
    {
        ...
        createInventoryTable(item,price);
     }
    ...
    for(auto iter = itemsQuantity.begin(); iter != itemsQuantity.end(); iter++)
    {
        ...
        updateCommodity(commodity->item(),
                        ...
                        iter.value(),
                        commodity->revenue());
        updateInventory(commodity->item(),commodity->price());
    }
